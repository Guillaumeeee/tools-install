...
openssl req -x509 -new -nodes -key /etc/nginx/ssl/webCA.key -sha256 -days 3650 -out /etc/nginx/ssl/webCA.pem -subj "/C=FR/ST=IDF/L=Paris/O=TIIX/OU=WTF/CN=webCA.tiix.lab"

# Create server certificate and key
openssl genrsa -out /etc/nginx/ssl/webServ.key 2048
openssl req -new -key /etc/nginx/ssl/webServ.key -out /etc/nginx/ssl/webServ.csr -subj "/C=FR/ST=IDF/L=Paris/O=TIIX/OU=WTF/CN=web.tiix.lab"
